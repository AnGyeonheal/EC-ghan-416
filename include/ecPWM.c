...
// 2. Configure GPIO AFR by Pin num.	
	...
	if(pin <= 7){
		port->AFR[0] |= 1UL << (4UL * pin);
	}
	else if( 7 < pin && pin <= 15){
		port->AFR[1] |= 1UL << (4UL * pin);
	}
...
// 4. Configure Timer Output mode as PWM
	...
	if(chN == 1){
		...
		TIMx->CCER &= ~TIM_CCER_CC1P;                       // select output polarity: active high	
		TIMx->CCER  |= TIM_CCER_CC1E;												// Enable output for ch2
	}
	else if(chN == 3){
		TIMx->CCMR2 &= ~TIM_CCMR2_OC3M;                     // Clear ouput compare mode bits for channel 3
		TIMx->CCMR2 |= 6UL << 4UL; 													// OC1M = 110 for PWM Mode 1 output on ch3
		TIMx->CCMR2	|= 1UL << 3UL;                 			    // Output 1 preload enable (make CCR3 value changable)	
		...
		TIMx->CCER |= 1UL << 0UL;														// Enable output for ch3
	}
	else if(chN == 4){
