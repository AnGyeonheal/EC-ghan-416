...
// 1. Select Timer channel(TIx) for Input Capture channel(ICx)
	...
	TIMx->CCMR1 |= 1UL << 8UL;  				     	//01<<8   CC2s    TI2=IC2
	TIMx->CCMR2 |= 1UL << 0UL;        				//01<<0   CC3s    TI3=IC3
	TIMx->CCMR2 |= 1UL << 8UL;  							//01<<8   CC4s    TI4=IC4
...
// 5.	Enable CCy Capture, Capture/Compare interrupt
	TIMx->CCER |= 1UL << 0UL;					// CCn(ICn) Capture Enable	

// 6.	Enable Interrupt of CC(CCyIE), Update (UIE)
	TIMx->CCER |= 1UL << 0UL;					// Capture/Compare Interrupt Enable	for ICn
...
// 2. Configure  IC number(user selected) with given IC pin(TIMx_CHn)
	switch(ICn){
			...
			case 3:
					...
					if (ICn==CHn) TIMx->CCMR2 |= 1UL << 0UL;				//01<<0   CC3S    Tx_Ch3=IC3
					else TIMx->CCMR2 |= 2UL << 0UL;		     				//10<<0   CC3S    Tx_Ch4=IC3
					break;
			case 4:
					TIMx->CCMR2 &= ~TIM_CCMR2_CC4S;										//reset   CC4S
					if (ICn==CHn) TIMx->CCMR2 |= 1UL << 8UL;	   				//01<<8   CC4S    Tx_Ch4=IC4
					else TIMx->CCMR2 |= 2UL << 8UL;	     					//10<<8   CC4S    Tx_Ch3=IC4
					break;
			default: break;
		}
