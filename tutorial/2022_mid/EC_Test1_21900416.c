...
void printsevensemgent(void){
    ...
    if(vel == 0){
        seven_segment_decode(0);
    }
    else if(vel == 0.5){
        GPIO_write(GPIOB, pin_a, 1);     // a
        GPIO_write(GPIOA, pin_b, 1);     // b
        GPIO_write(GPIOA, pin_c, 1);     // c
        GPIO_write(GPIOB, pin_d, 0);     // d
        GPIO_write(GPIOC, pin_e, 0);     // e
        GPIO_write(GPIOA, pin_f, 0);     // f
        GPIO_write(GPIOA, pin_g, 1);     // g
    }
    else if(vel == 1){
        GPIO_write(GPIOB, pin_a, 1);     // a
        GPIO_write(GPIOA, pin_b, 0);     // b
        GPIO_write(GPIOA, pin_c, 0);     // c
